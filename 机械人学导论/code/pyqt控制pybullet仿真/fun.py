...
class PyqtPanel:
    ...
    def create_control_panel(self, WRIST_DEFAULT_POSITION, WRIST_DEFAULT_EULER_ANGLE, ARM_DEAFULT_JOINT_ANGLE):
        try:
            win_app = QtWidgets.QApplication(argv)

            self.window = MainWindow()
            self.window.init_data(WRIST_DEFAULT_POSITION, WRIST_DEFAULT_EULER_ANGLE, ARM_DEAFULT_JOINT_ANGLE)
    ...
    def read_panel_sliders(self):
        return self.window.read_data()

    def sync_panel_posture_sliders(self, posture):
        self.window.sync_posture_data(posture)

    def sync_panel_jonit_sliders(self, angles):
        self.window.sync_joint_data(angles)
