...
from fun import set_wrist_pose, set_joint_angle, Draw, PyqtPanel
...
if __name__ == "__main__":
    try:
        ...
        # 新建一个子线程创建pyqt_panel
        panel = PyqtPanel()
        pyqt_panel_display = Thread(target=panel.create_control_panel,
                                    args=[[600, 0, 400], [180, 0, 0], [0, -90, 90, 0, 0, 0]])
        pyqt_panel_display.setDaemon(True)
        pyqt_panel_display.start()

        # 设置初始位姿
        set_joint_angle(pybullet, robot, joint_info, control_joints, joint_angles=ARM_DEAFULT_JOINT_ANGLE)

        last_angles = [0, -90, 90, 0, 0, 0]
        last_posture = [600, 0, 400, 180, 0, 0]
        # target_posture = [*WRIST_DEFAULT_POSITION, *WRIST_DEFAULT_EULER_ANGLE]
        while True:
            try:
                # 不断读取GUI滑动条数据(滑动条设置的是角度制)
                read_angles = panel.read_panel_sliders()["angles"]
                read_posture = panel.read_panel_sliders()["posture"]
                # 控制代码中使用的是弧度制，需要转换单位
                transform_angles = [radians(angle) for angle in read_angles]
                transform_posture = read_posture.copy()
                transform_posture[3:] = [radians(angle) for angle in read_posture[3:]]

                if read_angles != last_angles:
                    last_angles = read_angles

                    # 设置末端节点位姿并控制机械臂运动
                    analysis_posture = set_joint_angle(pybullet, robot, joint_info, control_joints,
                                                       joint_angles=transform_angles)

                    # 同步GUI滑条
                    # 约分减少精度(pyqt的滑动条无法设置浮点数)
                    analysis_posture = [round(value) for value in analysis_posture]
                    panel.sync_panel_posture_sliders(analysis_posture)
                    last_posture = analysis_posture

                    # 目标姿态
                    target_posture = analysis_posture
                elif read_posture != last_posture:
                    last_posture = read_posture

                    # 设置末端节点位姿并控制机械臂运动
                    analysis_angles = set_wrist_pose(pybullet, robot, joint_info, control_joints,
                                                     position=transform_posture[:3],
                                                     euler_angle=transform_posture[3:])

                    # 同步GUI滑条
                    # 约分减少精度(pyqt的滑动条无法设置浮点数)
                    analysis_angles = [round(value) for value in analysis_angles]
                    panel.sync_panel_jonit_sliders(analysis_angles)
                    last_angles = analysis_angles

                    # 目标姿态
                    target_posture = read_posture
                else:
                    continue

                target_pose = Posture()
                target_pose.set_position(*target_posture[:3])

                target_pose.set_euler_angle(*target_posture[3:])
