...
from math import pi
from fun import create_gui_posture_sliders, read_gui_sliders, set_wrist_pose, get_wrist_pose
...
if __name__ == "__main__":
    ...
    # 画出GUI滑动条
    posture_sliders = create_gui_posture_sliders(pybullet, WRIST_DEFAULT_POSITION, WRIST_DEFAULT_EULER_ANGLE)
    joint_info = dict()
    ...
    while True:
        try:
            ...
            # 不断读取GUI滑动条数据
            posture = read_gui_sliders(pybullet, posture_sliders)

            # 设置末端节点位姿并控制机械臂运动
            return_angles = set_wrist_pose(pybullet, robot, joint_info, control_joints, position=posture[:3],
                                           euler_angle=posture[3:])

            target_pose = Posture()
            target_pose.set_position(*posture[:3])
            target_pose.set_euler_angle(*posture[3:])
            # 绘制目标位姿坐标系
